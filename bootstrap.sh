...
do
    ...
    if [ -f "./catalog-migrate-${VERSION}/file-integrity-operator/catalog.json" ]; then
    ...
    if [[ "$VERSION" =~ ("4.12"|"4.13"|"4.14"|"4.15"|"4.16") ]]; then
        ...
        opm alpha render-template basic -o yaml "catalog/v${VERSION}/catalog-template.yaml" --migrate-level=bundle-object-to-csv-metadata > "catalog/v${VERSION}/file-integrity-operator/catalog.yaml"
    fi
